...
- **Agent 소스 선택**: oh-my-claudecode(OMC), 글로벌(`~/.claude/agents/`), 프로젝트별 Agent 소스 전환 지원
- **오케스트레이션 파이프라인**: 태스크 유형별 multi-stage 실행 (code → review + security + test 병렬)

## 고급 기능

- **CLI 세션 재개**: 작업별 Claude CLI 세션을 유지하고, 재실행 시 `--resume` 플래그로 이전 세션을 이어서 작업
- **프롬프트 히스토리**: 작업별 `prompt.md` 파일로 모든 프롬프트 기록 추적 및 디버깅 지원
- **AI 코드 리뷰 통합**: 오케스트레이션 Stage 2에서 코드 품질 리뷰, 보안 리뷰, 테스트를 병렬 실행
- **실시간 오케스트레이션 스트리밍**: 각 Agent 실행 중 thinking/tool 사용이 대시보드에 실시간 표시
...
```mermaid
graph LR
    subgraph Claude Conductor 시스템
        Dashboard["대시보드<br/>(React)<br/>Port 4000"]
        MCP["MCP Server<br/>(Node.js)<br/>Ports 3100/1"]
        Claude["Claude AI<br/>via stdin"]
        Redis["Redis<br/>(Docker)<br/>Port 6379"]
...
자세한 미구현 사항은 [spec/BACKLOG.md](./spec/BACKLOG.md) 참조.

- [ ] CHANGELOG 랜딩 뷰어 (대시보드 전용 페이지)
- [ ] BACKLOG 전환 시 자동 롤백 (git stash)
- [ ] GitHub PR 자동 생성 연동
- [ ] 작업 메트릭스 수집 (시간, 비용, 토큰)
- [ ] 대시보드 분석 차트
- [ ] Slack/Discord 알림 연동
